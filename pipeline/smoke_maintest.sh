...
#
# 사용:
#   bash smoke_maintest.sh              # 군 대표 + 다중사슬 타깃 자동 선정, Phase A만
...
[ -f "$CSV" ]  || { echo "!! $CSV 없음. 먼저 pick_maintest_depth.py"; exit 1; }
...
for T in $TARGETS; do
  ...
  # 조성 생성(GPU 없이)
  GEN_ONLY=1 RUNG="$RUNG" TARGET="$T" REPLICAS="$NCOMP" bash comp_x_reps.sh >"/tmp/smk_gen_$T.log" 2>&1
...
if [ $GPU -eq 0 ]; then
  echo; say "GPU 검증까지 하려면:  bash smoke_maintest.sh --gpu"; exit 0
fi
