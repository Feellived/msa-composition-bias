...
pose_features.py docstring이 지정한 Phase 1: "복합체 내부에서 각 피처로 pose 순위 → ipTM·무작위·
...
사용(stdlib only):
  python phase1_rerank.py                                   # dockq, scope=rung0(배포)
  python phase1_rerank.py --label recall                    # 에피토프 위치 라벨
  python phase1_rerank.py --scope all                       # 전 pose(깊이 섭동 포함=다양성↑)
  python phase1_rerank.py --csv 다른.csv --succ 0.23
"""
