...
■ 왜
  eval_topk.py 가 낸 표는 아직 기술 통계다. 특히 정제된 후보(sites_refined)의 상위 3개에서
...
■ 무엇을 재나 — '쓸 만한 후보'가 상위 k개 안에 있는가(이진)
  test_selectors.py 는 평균 F1 을 검정했지만, 여기서는 **쓸만함 비율**
  (진짜 결합자리를 site_reproducibility.py 와 같은 기준 0.5 이상 덮는 후보가 상위 k개 안에
  있는가)을 검정한다. eval_topk.py §6 의 실용 기준(≥0.70)이 재는 것과 같은 지표다.

  귀무가설 = "상위 k개를 무작위로 뽑아도 이만큼 쓸 만한 게 걸린다".
  타깃마다 무작위로 k개를 뽑는 걸 nperm 번 반복해 평균 쓸만함 비율의 귀무분포를 만들고,
  선택기의 관측값이 그 분포의 어디쯤인지 본다(단측, site_reproducibility.py 의 뒤섞기 검정과 같은 방식).

  ⚠️ 다중비교: 이 진단은 eval_topk.py 표에서 상위 3개 성능이 좋아 보인 것들만 검정한다.
     사후에 좋아 보이는 걸 고르는 것 자체가 다중비교이므로, 표에 나온 선택기 전부를
     Bonferroni 로 같이 보정한다(0.05 / 검정한 선택기 수).

사용 (conda activate boltz · pipeline/ 에서):
  python -u test_topk.py --sites results/sites_refined \
