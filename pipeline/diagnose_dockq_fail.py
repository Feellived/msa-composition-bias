...
dockq_one.py 가 쓰는 native_merged / pose_merged 는 항원·항체 사슬을
chains.json 의 서열과 가장 잘 맞는 사슬로 골라 붙인다(dockq_sweep.py 의 best()).
...
사용(DockQ+biopython env · pipeline/ 에서):
  python -u diagnose_dockq_fail.py --target 9b7g_QP \
      --pose <AF3 예측 cif 경로> \
      [--pose <자세 2> --pose <자세 3> ...]
