...
원인: `neff_ladder.py`의 옛 `read_raw`가 ColabFold a3m 첫 줄의 메타 주석을 서열로 취급하고
...
사용:
  python fix_a3m_query.py                        # dry-run(기본): 몇 개가 어떻게 바뀌는지만
  python fix_a3m_query.py --apply                # 실제 수정
  python fix_a3m_query.py --roots /경로1 /경로2   # 대상 폴더 지정
"""
...
def main():
    ...
    if a.apply and n_fix:
        print("\n✅ 복구 완료. 검증: python check_msa_match.py")
