...
#   완료 후: python collect_iptm.py --data "$DATA/decoy/protenix" --out results/decoy_iptm.csv
# ══════════════════════════════════════════════════════════════════════════════
set -uo pipefail
cd "$(cd "$(dirname "$0")" && pwd)"
...
OUTROOT="${OUTROOT:-$DATA/decoy/protenix}"    # ⚠️ collect_iptm.py 가 경로에서 리터럴 "/protenix/<타깃>/<깊이>/<실행>/results/" 를 찾는다
RUNGS="${RUNGS:-0 4 8}"          # 항원 사다리에서 재사용할 rung 번호 (낮음·중간·높음)
SAMP="${SAMP:-5}"; SEED="${SEED:-0}"; SMOKE="${SMOKE:-0}"
say(){ echo "[$(date '+%m-%d %H:%M:%S')] $*"; }

[ -f "$MANIFEST" ] || { say "!! 없음: $MANIFEST — decoy_pairs.py --write 를 먼저 실행할 것"; exit 1; }
...
while IFS=, read -r decoy ag ag_grp ab ab_grp; do
  ...
  if [ ! -f "$cj" ]; then say "! $decoy: chains.json 없음 — decoy_pairs.py --write 확인"; continue; fi
  ...
  for r in $RUNGS; do
    ...
    # ⚠️ collect_iptm.py 의 정규식이 <타깃>/<깊이>/<실행>/results/ 3단을 기대한다 —
...
say "다음: python collect_iptm.py --data \"$DATA/decoy/protenix\" --out results/decoy_iptm.csv"
