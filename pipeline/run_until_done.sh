...
#   · 채점(analyze_target.sh)은 DockQ 환경이 필요하므로 conda run 으로 부른다.
...
busy(){ pgrep -f "run_maintest.sh --apply" >/dev/null || pgrep -f "comp_x_reps.sh" >/dev/null; }
if busy; then
...
if [ "$AUTO_ANALYZE" != "1" ]; then
  say "예측 완료. 채점은 직접: conda activate $DOCKQ_ENV && bash analyze_target.sh <타깃들>"
  exit 0
fi
...
fail=0
for t in "${TGTS[@]}"; do
  conda run -n "$DOCKQ_ENV" --no-capture-output bash analyze_target.sh "$t" || { fail=$((fail+1)); say "  ! $t 채점 실패"; }
done
say "채점 끝 (실패 ${fail}개). 다음: conda run -n $DOCKQ_ENV python collect_results.py"
[ "$fail" -eq 0 ] || exit 5
