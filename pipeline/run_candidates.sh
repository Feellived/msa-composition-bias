...
#            (comp_x_reps.sh가 조성 a3m 생성·seedfull 준비까지 알아서 함)
#
...
while IFS=, read -r target model rung comps reps full_reps note; do
  ...
  RUNG="$rung" TARGET="$target" MODEL="$model" COMPS="$comps" REPS="$reps" bash comp_x_reps.sh \
    || say "  !! ① 실패 $target (계속 진행)"
  RUNG="$rung" TARGET="$target" MODEL="$model" COMPS="full" REPS="$full_reps" bash comp_x_reps.sh \
    || say "  !! ② 실패 $target (계속 진행)"
...
if [ "$ANALYZE" = 1 ] && [ "$DRY" != 1 ] && [ -n "$done_list" ]; then
  ...
  bash analyze_target.sh $done_list
else
  say "분석은 따로:  bash analyze_target.sh$done_list"
fi
