...
사용:
  python import_rbd.py --stage undo            # 먼저 정리(확인 후 --apply)
  python import_rbd.py --stage csv --apply
  python prep_targets.py --csv rbd_offhot.csv --struct structures --outdir targets
  python import_rbd.py --stage msa --apply
  python check_msa_match.py --only 8sis_HL ...  # 반드시 게이트로 확인
"""
...
def stage_msa(a, tg):
    ...
    print("\n다음: python check_msa_match.py   (반드시 게이트로 확인)  →  python neff_ladder.py")
...
def stage_undo(a, tg):
    ...
    print("\n정리 완료. 다음: python import_rbd.py --stage csv --apply")
