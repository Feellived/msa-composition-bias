#!/usr/bin/env bash
# ══════════════════════════════════════════════════════════════════════════════
# [지표 통일 ②] seed_vs_comp.py 를 30종 전부에 대해 돌린다.
#
# 왜 27종이었나 — seed_vs_comp.py 는 타깃 폴더 밑에 깊이(depth) 하위폴더가 정확히
...
# merge_seedcomp.py 로 하나의 표와 30종 기준 요약(부호검정 포함)을 만든다.
#
# 사용 (conda activate boltz · pipeline/ 에서):
#   bash rerun_seedcomp_all.sh 2>&1 | tee results/rerun_seedcomp_all.log
...
echo "→ results/seedcomp_by_target/*.csv 30개. 이어서: python -u merge_seedcomp.py"
