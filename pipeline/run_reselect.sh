...
[ -f "$ABE" ] || { echo "!! $ABE 없음 — score_abepitope.py 먼저"; exit 1; }

echo "[1/3] 선택기 '$SEL' 로 후보 고르기"
python eval_selectors.py --sites results --abepi "$ABE" \
...
PY
  ...
  echo "── $t (후보 $CAND) ──"
  for arm in $ARMS; do
    python scripts/sites_to_pocket.py --sites "$MSAD/results/sites_${t}.json" \
...
python scripts/dockq_demo.py --targets "$TGTS" --models "$MODELS" \
