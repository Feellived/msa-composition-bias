...
prep_manifest.py의 항원군 분류(RBD/HA/Env)를 재사용하되 날짜 필터를 반전. 각 테스트 계면의
'컷 이전 near-duplicate 이웃수'를 세기 위한 참조 풀. 출력 = ref_manifest.csv → prep_fetch_structures.py로 구조 다운.
...
사용: python prep_idist_ref_manifest.py [--cut 2023-06-01] [--max-per-family 1200]
"""
import argparse, re
import pandas as pd
...
def main():
    ...
    print(f"  다음: python prep_fetch_structures.py --manifest {a.out} --outdir ref_structures")
