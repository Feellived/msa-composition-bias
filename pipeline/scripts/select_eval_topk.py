...
■ (1) 상위 k개 — 산출물의 형태를 바꾼다
  선택기 9개가 전부 무작위와 구별되지 않았다(select_test_selectors.py). 그러면 하나 고르기를
...
■ (2) 자세 수 편향 보정 — 지금 결론을 뒤집을 수 있는 유일한 재분석
  ...
  ⚠️ 이미 선택기 9개를 검정했다. **새로 검정할 것은 이 둘뿐**이라고 미리 정해 두었다
     ...
     유의성 자체는 select_test_selectors.py 가 낸다. 여기서는 점수만 만든다.

■ 안전장치
  기존 `select_eval_selectors.py` 는 건드리지 않는다(그 숫자가 이미 보고서에 들어가 있다).
...
def main():
    ...
    print("   유의성은 select_test_selectors.py 가 따로 답한다(새로 검정할 선택기는 "
