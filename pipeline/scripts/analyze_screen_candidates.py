...
사용:
  python analyze_screen_candidates.py                       # protenix, DockQ 기준
  python analyze_screen_candidates.py --label recall        # 결합자리 회복률 기준
  python analyze_screen_candidates.py --model boltz         # boltz 재실행 후
  python analyze_screen_candidates.py --top 15
"""
    ...
    if todo:
        ...
        for r in todo:
            print(f"    {r['target']:12} 등급{r['tier']}  개선폭 {r['gain']:+.3f}  "
                  f"→  RUNG={r['peak_rung']} TARGET={r['target']} bash make_composition_reps.sh")
