...
같은 형식(path,target,run,pose,score)으로 저장한다 → select_eval_selectors.py 가 그대로 먹는다.

왜 — 4.6절은 "선택기 넷 중 ipTM 은 우연과 구별되지 않았다"고 쓰지만, 선택기 비교표
(select_eval_selectors.py)에는 ipTM 항목이 없다. 같은 판에서 재보려면 자세별 ipTM 이 필요하다.
...
사용 (conda activate boltz · pipeline/ 에서):
  python -u analyze_collect_iptm.py --data $DATA/compreps/seedrep_cand/protenix \
