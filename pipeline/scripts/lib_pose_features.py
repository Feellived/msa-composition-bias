...
경로 규약은 eval_dockq_sweep.py / eval_epitope_shift.py와 동일:
  pose = $DATA/<model>/<t>/rung<r>/results/**/*.cif · native·chains.json = targets/<t>/

사용(DockQ + biopython + scipy env):
  python lib_pose_features.py --models boltz protenix --out results/pose_features.csv
...
from lib_epitope_recall import (load as er_load, best as er_best,
