...
사용(DockQ env):
  ...
  python analyze_site_reproducibility.py --csv results/compreps_8ulr_HL.csv --data $DATA/compreps
  python analyze_site_reproducibility.py --csv ... --link 0.6 --nperm 5000
"""
...
def main():
    ...
    if mode == "abepi":
        ...
        if not AB:
            raise SystemExit(f"!! {tgt} 의 AbEpiScore 행이 없다 — {a.abepi}\n"
                             f"   eval_abepitope.py 로 먼저 채점할 것")
    ...
    for ci, ix in enumerate(cl, 1):
        ...
        # 유도 재도킹은 여기서 1-based 서열 위치로 바꿔 쓴다(make_pocket_from_sites.py).
        sites.append(dict(cand=ci, n_comp=len(ix), comps=names.split(","),
                          ...
                          #    select_site.py 는 읽을 때 이 키를 버리고 그 사실을 화면에 알린다.
