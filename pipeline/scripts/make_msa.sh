...
#   bash make_msa.sh            # 전체
#   ONLY=8q7s_O bash make_msa.sh  # 한 타깃만(smoke)
...
while IFS=, read -r target pdb group ab dirtype ag_chains label; do
  ...
  if [ "$dirtype" = "diverse" ]; then
    ...
    python "$HERE/prep_ladder.py" --a3m "$src" --outdir "$od" --rungs "$RUNGS" && say "ladder(C) $target ← $(basename "$src")"
  else
...
open('$fa','w').write('>${target}_${c}\n'+sm['$c']+'\n')"
      ...
      python "$HERE/prep_ladder.py" --a3m "$a3m" --outdir "$od" --rungs "$RUNGS" && say "ladder $target/$c ← $(basename "$a3m")"
    done
  fi
done < <(tail -n +2 "$LIST")
