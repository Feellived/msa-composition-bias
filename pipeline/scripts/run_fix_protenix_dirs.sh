...
#
# 사용:
#   bash run_fix_protenix_dirs.sh              # 무엇을 지우고 옮길지만 출력
...
if [ $APPLY -eq 0 ]; then
  echo "→ 실제로 수행하려면:  bash run_fix_protenix_dirs.sh --apply"
  exit 0
fi
...
2026-07-28        run_fix_protenix_dirs.sh --apply 로 정리:
...
⚠️ lib_pose_features.py 를 --rescore 로 돌리지 말 것(이미 채점된 자세를 다시 계산할 이유가 없다).
