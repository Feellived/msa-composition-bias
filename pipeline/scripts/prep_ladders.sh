...
#       prep_import_rbd.py --stage msa 가 만들거나, make_msa.sh 산출물을 놓아둔 상태.
#
# 사용:
#   bash prep_ladders.sh                 # dry-run: 무엇을 만들지만 출력
#   bash prep_ladders.sh --apply
#   ONLY="8sis_HL 8p5m_GL" bash prep_ladders.sh --apply
#   RUNGS=12 bash prep_ladders.sh --apply
# ══════════════════════════════════════════════════════════════════════════════
set -uo pipefail
cd "$(cd "$(dirname "$0")" && pwd)"
...
while IFS=, read -r target pdb group ab dirtype ag_chains label; do
  ...
  for c in "${AGC[@]}"; do
    ...
    python prep_ladder.py --a3m "$d/full_backup.a3m" --outdir "$d" \
...
if [ $APPLY -eq 0 ]; then
  echo "→ 실제로 만들려면:  bash prep_ladders.sh --apply"
else
  echo "→ 다음: python prep_a3m_check_match.py   (사다리 칸까지 확인하려면 --rung rung5 등)"
