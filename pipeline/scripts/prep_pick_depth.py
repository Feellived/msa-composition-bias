...
⚠️ 규칙은 **결과를 보기 전(2026-07-28)에 확정**한 것이다. 여기서 바꾸지 말 것.
  ...
  각 칸마다 구조 5개의 '진짜 결합자리 덮음'(recall)을 보고 0.4 이상인 개수를 센다.
    ...
    · ⚠️ rung0은 후보에서 제외한다(2026-07-28 추가). rung0 = 원래 MSA 전체이고
      make_composition_reps.sh가 그것을 그대로 대조군(seedfull.a3m)으로 쓴다. 거기서 조성을 재추첨하면
...
사용(stdlib only):
  python prep_pick_depth.py                        # 전체
  python prep_pick_depth.py --group RBD            # 코로나 세트만
  python prep_pick_depth.py --only 8sis_HL 9zdu_HL
  python prep_pick_depth.py --out maintest.csv     # 실행기가 읽을 명단

--only 로 지정한 이름이 표에 안 나오면 이유를 구분해 알린다(조용히 빈 표를 내지 않는다):
  · sweep_targets.csv 에는 있으나 pose_features.csv 에 없음 → 아직 채점 안 됨(먼저 lib_pose_features.py)
...
def main():
    ...
    if a.only:
        ...
        if no_score:
            print(f"⚠️ 채점 결과가 없는 타깃 {len(no_score)}개 — 먼저 "
                  f"lib_pose_features.py --models {a.model} 를 돌릴 것:")
            print("   " + " ".join(no_score) + "\n")
    ...
    for t in tgts:
        ...
        # rung0 = 원래 MSA 전체(make_composition_reps.sh가 rung0.a3m을 그대로 seedfull.a3m으로 복사한다).
