...
기존 선별(analyze_screen_candidates.py)은 DockQ(자세 점수)로만 골랐다. 그런데 결합 자리를 맞혀도
...
사용(stdlib only):
  python analyze_screen_epitope_shift.py                      # protenix
  python analyze_screen_epitope_shift.py --model boltz        # boltz 재실행 후
  python analyze_screen_epitope_shift.py --min-shift 0.15
"""
