...
같은 23종 기준(후보가 2개 이상이라 실제로 고를 게 있는 타깃 — select_eval_selectors.py 의
...
정의 (본문 3.1절의 "절반 이상 덮는다"와 통일):
  ...
  유의성 = 순열검정(타깃마다 후보를 무작위로 다시 뽑기를 반복해 우연히 이 이상 성공하는 빈도를 잰다),
           analyze_site_reproducibility.py 의 뒤섞기 검정과 같은 방식.
...
def main():
    ...
    for p in sorted(glob.glob(os.path.join(a.sites, "sites_*.json"))):
        ...
        if len(d.get("candidates", [])) >= a.min_cand:
            T.append(d)
    if not T:
        raise SystemExit("!! 후보 2개 이상인 타깃이 없다 — run_rerun_sites.sh 먼저 돌렸는지 확인")
