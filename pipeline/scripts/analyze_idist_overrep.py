...
사용(ppiref env, 서버):
  # 0) 레퍼런스 구조 준비: python prep_idist_ref_manifest.py  →  python prep_fetch_structures.py --manifest ref_manifest.csv --outdir ref_structures
  python analyze_idist_overrep.py --stage extract           # 테스트+레퍼런스 계면 추출(재사용 캐시)
  python analyze_idist_overrep.py --stage score             # 임베딩→이웃수→ results/overrep_idist.csv
  (또는 --stage all 로 한 번에)
"""
...
def do_score(a):
    ...
    print(f"\n→ {a.out} ({len(out)}행). 주 신호 = min_dist(작을수록 과대표집)·mean_knn·frac_ndup. "
          f"다음: analyze_idist.py 로 A/B 대비 + 깊이-취약성 상관.")
