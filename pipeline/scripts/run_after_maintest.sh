...
#     run_analyze_target.sh 는 CSV 가 있으면 건너뛰므로, 옛 실험의 낡은 CSV 가 남아 있으면
...
#
# 사용:
#   bash run_after_maintest.sh                     # 무엇을 할지만 출력(dry-run)
#   bash run_after_maintest.sh --apply             # 대기 → 채점 → 후보 자리
#   SCREEN=1 bash run_after_maintest.sh --apply    # 위 + boltz 'ours' 훑기까지
#   NOWAIT=1 bash run_after_maintest.sh --apply    # 기다리지 않고 지금 끝난 것만 처리
#
...
# conda run 은 PATH 처리에 변덕이 있어 make_composition_reps.sh 와 같은 방식(source + activate)을 쓴다.
...
if ! command -v DockQ >/dev/null 2>&1; then
  ...
  say "   다른 env 면: DOCKQ_ENV=<이름> bash run_after_maintest.sh --apply"
  exit 1
fi
say "채점 env = $DOCKQ_ENV · DockQ = $(command -v DockQ)"

# ── 1) 예측이 끝나기를 기다린다 ───────────────────────────────────────────────
busy(){ pgrep -f "make_composition_reps.sh" >/dev/null || pgrep -f "run_maintest.sh --apply" >/dev/null; }
...
for r in csv.DictReader(open(sys.argv[1])):
    ...
    # ⚠️ compreps CSV 만 보면 안 된다. run_analyze_target.sh 는 채점(dump) → 통계 → 결합자리 순으로
...
while IFS=$'\t' read -r t act have want deps miss; do
  ...
  if [ "$act" = "new" ]; then
    ...
    bash run_analyze_target.sh "$t" </dev/null && NEW=$((NEW+1)) || FAIL=$((FAIL+1))
  else
    say "[$I/$TODO] $t 재채점 시작 (실행 $have/설계 $want · 깊이 $deps${miss:+ · 없던 파일 $miss}) · 경과 $(( (TS-T0)/60 ))분"
    REDO=1 bash run_analyze_target.sh "$t" </dev/null && REDONE=$((REDONE+1)) || FAIL=$((FAIL+1))
  fi
...
if [ "$SCREEN" = "1" ] && [ -n "$TGTS" ]; then
  if [ -d "$CD" ]; then
    ...
    ( cd "$CD" && python scripts/eval_demo.py --targets "$TGTS" --models boltz ) || say "! 채점 중 오류"
  else
    say "! $CD 없음 — 훑기 건너뜀"
  fi
else
