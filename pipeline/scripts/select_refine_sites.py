...
■ 왜 (2026-08-01 발견)
  ...
  넓어지는 직접 원인은 analyze_site_reproducibility.py 의 후보 조립이
...
사용 (msa-depth pipeline 폴더에서, DockQ env 불필요·CPU):
  python select_refine_sites.py --targets "8k3k_D 8ulr_HL 8sis_HL"        # 기본 sweep
  python select_refine_sites.py --all --out results/refine_sweep.csv
  python select_refine_sites.py --all --dump-sites results/sites_refined --use 0.7,0.5,30
      → 그 설정으로 sites_*.json 재생성. 이어서
        python select_eval_selectors.py --sites results/sites_refined --abepi <abepiscore_all.csv>
...
import lib_pose_features as PF              # native_true
...
def dump_sites(loaded, cf, mf, mr, link, outdir):
    """정제한 후보를 analyze_site_reproducibility.py --dump-sites 와 **같은 형식**으로 쓴다.

    이걸 select_eval_selectors.py 에 넘겨야 "정제하면 선택기가 실제로 좋아지나"를 잴 수 있다.
    ...
    print("  다음: python select_eval_selectors.py --sites "
...
def main():
    ...
    ap.add_argument("--dump-sites", default="",
                    help="정제한 후보를 sites_<타깃>.json 으로 이 폴더에 쓴다 "
                         "(analyze_site_reproducibility.py --dump-sites 와 같은 형식 → select_eval_selectors.py 로 바로 넘어간다)")
    ...
    print("\n⚠️ 이건 천장이다. 실제로 그 후보를 고를 수 있는지는 select_eval_selectors.py 가 따로 답한다.")
