...
run_analyze_target.sh 를 돌리면 타깃당 파일이 대여섯 개씩 생겨 29개 타깃이면 백 개가 넘는다.
...
사용:
  python analyze_collect_results.py
  python analyze_collect_results.py --metric recall     # 화면 표만 이 지표로
"""
...
def main():
    ...
    if not rows:
        raise SystemExit(f"!! {a.dir} 에 summary_*_*.csv 가 없다.\n"
                         "   run_analyze_target.sh 를 먼저 돌릴 것"
