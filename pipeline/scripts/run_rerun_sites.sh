...
#
# 이 스크립트는 analyze_site_reproducibility.py 를 --dump-sites 를 붙여 그대로 다시 돌릴 뿐,
# 새 로직은 없다 — run_analyze_target.sh ④단계와 완전히 같은 호출이다.
#
...
for T in $TARGETS; do
  ...
  echo "== $T =="
  python -u analyze_site_reproducibility.py --csv "$csv" --data "$DATA/compreps" \
