...
#   → 채점(eval_seedrep_cand.py)에서 "그 깊이서 관측했던 peak(예 8ulr 0.66)이 N seed에서 재현되나
...
while IFS=, read -r target model peak_rung replicas obs_dq obs_rec; do
  ...
  for i in "${!AGC[@]}"; do
    ...
    python run_seed_replicate.py --a3m "$full" --depths "$depth" --replicas "$replicas" \
...
say "완료. 채점: python eval_seedrep_cand.py  (DockQ env)"
