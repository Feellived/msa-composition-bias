...
사용 (DockQ가 있는 환경이면 --score 도 가능)
  python run_watch_maintest.py                 # 한 번 출력
  python run_watch_maintest.py --loop          # 60초마다 갱신 (Ctrl+C 종료)
  python run_watch_maintest.py --loop --every 300
  python run_watch_maintest.py --score 8t4d_OQ # 지금까지 나온 것만 채점(CPU, 몇 분)

--score 는 eval_dump_seedrep.py 를 그대로 부른다. 아직 안 끝난 타깃도 그 시점까지의
...
def depth_of(data, listcsv, target, rung):
    """깊이 폴더 이름 d<서열수>. 생성기(make_composition_reps.sh)와 똑같이 a3m의 '>' 줄을 센다.
    ...
    if not os.path.exists(f):
        return None
    # 생성기(make_composition_reps.sh)는 grep -c '^>' 로 센다. 같은 계산을 파이썬으로 다시 구현하면
...
def main():
    ...
    if a.score:
        ...
        cmd = ["python", "eval_dump_seedrep.py", "--data", os.path.join(a.data, "compreps"),
    ...
    while True:
        ...
        print("\n채점을 미리 보려면:  python run_watch_maintest.py --score <타깃>   (DockQ 환경)")
        if not a.loop:
            return
        time.sleep(a.every)
