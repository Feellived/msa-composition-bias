...
#
# ⭐ 조성 a3m 은 **다시 만들지 않는다.** make_composition_reps.sh 가 없을 때만 만들고, protenix 실행 때
...
#   python eval_dump_seedrep.py --data $DATA/compreps --only <타깃> \
...
# ── 항원 사슬 명단 준비 ───────────────────────────────────────────────────────
# make_composition_reps.sh 는 항원 사슬을 $LIST(기본 sweep_targets.csv)의 ag_chains 열에서 읽고,
...
PY
  ...
  MODEL=boltz LIST="$TMPLIST" RUNG="$RUNG" TARGET="$T" REPLICAS="$NCOMP" COMPS="$COMPLIST" REPS="$NREPS" \
    bash make_composition_reps.sh </dev/null || say "  ! 조성 실행에서 오류 — 로그 확인"

  say "  ② 원래 MSA $NFULL회 (boltz)"
  MODEL=boltz LIST="$TMPLIST" RUNG="$RUNG" TARGET="$T" COMPS="full" REPS="$NFULL" \
    bash make_composition_reps.sh </dev/null || say "  ! 원래 MSA 실행에서 오류 — 로그 확인"
done
...
if [ $APPLY -eq 1 ]; then
  ...
  for T in $TARGETS; do
    ...
    echo "  python eval_dump_seedrep.py --data \$DATA/compreps --only $T --cand-first \\"
    echo "         --cand cand_boltz_${T}.csv --csv-out results/compreps_${T}_boltz.csv"
  done
  say "그다음 analyze_site_reproducibility.py --csv results/compreps_<타깃>_boltz.csv --dump-sites 로 후보 자리를 뽑는다."
else
