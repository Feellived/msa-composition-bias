...
사용(DockQ env):
  python eval_dump_seedrep.py                      # 전체
  python eval_dump_seedrep.py --only 8ulr_HL       # 한 후보만
  python eval_dump_seedrep.py --csv-out results/seedrep_poses.csv
"""
...
def main():
    ...
    if a.only:
        ...
        if got is not None:
            ...
            print(f"   본 검정 타깃이면 prep_pick_depth.py 로 {a.maintest} 를 먼저 만들 것.")
            raise SystemExit(2)
