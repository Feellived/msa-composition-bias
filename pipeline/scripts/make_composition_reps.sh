...
#
# 출력은 seedrep_cand와 같은 구조 → eval_dump_seedrep.py로 그대로 채점.
#   폴더명 seed<조성>_r<반복>   (예: seed2_r3, seedfull_r7)
#
# 사용:
#   # 8ulr 본실험(조성 8개 × 4회)
#   bash make_composition_reps.sh
#   # 원래 MSA 10회(대조)
#   COMPS="full" REPS=10 bash make_composition_reps.sh
#   # 다른 복합체 예비검정(조성 5개 1회씩 + 원래 5회) — RUNG 주면 조성 a3m을 자동 생성
#   RUNG=3 TARGET=9azr_HL COMPS="0 1 2 3 4" REPS=1 bash make_composition_reps.sh
#   RUNG=3 TARGET=9azr_HL COMPS="full"      REPS=5 bash make_composition_reps.sh
#   # 조성 a3m만 만들고 GPU 실행 없이 종료(스모크 검증용)
#   GEN_ONLY=1 RUNG=2 TARGET=8t4a_PR REPLICAS=6 bash make_composition_reps.sh
# ══════════════════════════════════════════════════════════════════════════════
set -uo pipefail
cd "$(cd "$(dirname "$0")" && pwd)"
...
for i in "${!AGC[@]}"; do
  ...
  if [ -n "$RUNG" ]; then
    ...
    if ! ls "seedrep_cand/${TARGET}_${ch}/d${d}"/seed[0-9]*.a3m >/dev/null 2>&1; then
      ...
      python run_seed_replicate.py --a3m "$f0" --depths "$d" --replicas "$REPLICAS" \
...
say "채점: python eval_dump_seedrep.py --data $DATA/compreps --only $TARGET --csv-out results/compreps_${TARGET}.csv"
