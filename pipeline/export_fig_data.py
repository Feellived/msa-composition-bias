...
⚠️ 재랭커 아님 — 그림 재료만 만든다. stdlib only. 서버 results/ 에서 실행하고
  ...
  python export_fig_data.py                 # results/pose_features.csv + results/crosscheck_depth.csv
  python export_fig_data.py --pf 다른.csv --cc 다른.csv
...
def main():
    ...
    try:
        ...
        print("(crosscheck_depth.csv 없음 — python crosscheck_depth.py 먼저 실행)")
