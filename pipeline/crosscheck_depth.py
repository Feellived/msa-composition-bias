...
⚠️ 이건 재랭커(pose 선택)가 아니라 **깊이-반응 케이스 스터디**다. pose_features.csv 하나로 됨(두 CSV join 불필요):
   ...
   epitope_shift.py와 동일 계산(pose_features가 그걸 import). rung0 = full MSA(가장 깊음), rung↑ = 얕음.
...
사용(stdlib only, 어디서나):
  python crosscheck_depth.py                         # results/pose_features.csv, 전 타깃
  python crosscheck_depth.py --focus                 # 후보만(8txu_HL·9y0a_AB·9azr_HL·RBD앵커)
  python crosscheck_depth.py --min-gain 0.15         # responsive 문턱
  python crosscheck_depth.py --targets "8txu_HL 9y0a_AB"
