...
여기서는 **제약 없이 돌린 예측 그 자체**에서 신호를 더 캐낸다. 전부 정답이 필요 없다.
  ...
  ④ 후보개수   — 이미 쓰고 있는 것 (diagnose_harm.py 결과에서 읽는다)
...
사용 (conda activate boltz · pipeline/ 에서):
  python -u gate_signals.py --selftest      # 통계 부분만 자체 검증 (자료 없이 즉시)
...
def main():
    ...
    import pose_features as PF          # 자체 검증에는 필요 없으므로 여기서 불러온다
    ...
    if not os.path.exists(a.diag):
        raise SystemExit(f"!! {a.diag} 가 없다 — diagnose_harm.py 를 먼저 돌릴 것")
