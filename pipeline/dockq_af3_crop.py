...
diagnose_dockq_fail.py 로 확인한 원인: 9b7g_QP 의 항원은 잘 맞는데(덮음 0.96),
AF3 에 제출한 항체 서열이 정답 구조가 담은 가변 영역(H 115 · L 109잔기)보다
약 2배 길다(H 242 · L 216잔기, 정렬점수 0.19~0.23). dockq_sweep.py 의 pose_merged
...
사용(DockQ+biopython env · pipeline/ 에서):
  python -u dockq_af3_crop.py --target 9b7g_QP \
      --pose /home/admuser/projects/bk21-antibody-ml/consensus_docking/fold_9b7g_qp_model_0.cif
"""
import argparse
import json
import os
