...
읽는 파일 (analyze_target.sh 가 만든 것):
...
사용:
  python score_criteria.py
  python score_criteria.py --set3 "8k3k_D 8k46_I ..."      # 세트 3 명단(빈도 계산용)
  python score_criteria.py --exclude 8ulr_HL --csv-out results/criteria.csv
"""
...
def main():
    ...
    if n == 0:
        raise SystemExit("!! 채점된 복합체가 없다 — analyze_target.sh 를 먼저 돌릴 것")
