...
#       출력 구조를 seedrep_cand와 똑같이 만들어 dump_seedrep_full.py로 바로 채점.
#
# 사용:
#   bash repro_test.sh                       # 8ulr_HL protenix seed0 기준 3회
#   TARGET=9y0a_AB MODEL=boltz SRCSEED=seed0 REPS=3 bash repro_test.sh
#   # 채점: python dump_seedrep_full.py --data $DATA/repro --only 8ulr_HL
# ══════════════════════════════════════════════════════════════════════════════
set -uo pipefail
cd "$(cd "$(dirname "$0")" && pwd)"
...
say "완료. 채점:  python dump_seedrep_full.py --data $DATA/repro --only $TARGET"
