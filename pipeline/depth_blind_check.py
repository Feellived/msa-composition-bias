...
■ 왜 이걸 묻나
  본 검정에 쓴 깊이는 `pick_maintest_depth.py` 가 **정답 대비 recall 을 보고** 골랐다(칸마다
...
■ 무엇을 재나
  ...
  0.4 는 `pick_maintest_depth.py` 의 SUCC_RECALL 과 같은 값이며 **결과를 보기 전에 확정**됐다.
...
사용 (stdlib only · CPU 몇 초):
    ...
    python depth_blind_check.py
    python depth_blind_check.py --thr 0.40 --model protenix --out results/depth_blind_check.csv
"""
import argparse
import csv
import math
