...
| `~/projects/bk21-antibody-ml/consensus_docking/` | 유도 재도킹(데모) · 물리 도킹 · 모델 비교 | `results/*.csv` · `dataset/*.csv` 커밋됨 |
...
| 2.1 | 5모델 × 5복합체 DockQ | `consensus_docking/results/dockq_cofolder.csv` · `summary.csv` | — |
...
| **5.7 · 6.2** | 데모 네 조건 최고·중앙 | `consensus_docking/results/demo_dockq.csv` | `arm,n_pose,dockq_max,dockq_med,recall_max,recall_med` |
...
| 4.1 | 명단의 원본 956 후보 | `consensus_docking/dataset/manifest_labeled.csv` | 956행 · `antigen,epitope_class,n_epi,frac,epitope_residues` |
...
- **`dump_seedrep_full.py`는 `--data $DATA/compreps` 필수.** 빼면 옛 `seedrep_cand`(무효 boltz 3건)를
